...
#bootstrap the predictions score N=1000
#calculate the C score and hoslem on each boot --> save into intermediate file
#calculate the average 
#calculate bayes factor between 2 submissions
...
#scoring 
teststats <-  function(predicted,S) {
  ...
  hoslem_p <- HosLem.test(S,predicted)#plot="final_phase_norange") # fit$y is the actual survival object that is needed 
...
#obtaining 1000 bootstrap samples 
set.seed(123456)
#resamples <- lapply(1:999, function(i) sample(data_merge$Score, replace = T))
##calculate the score for each boots
#evaluation <- lapply(resamples, teststats, data_merge$True.Score)
evaluation_2 <- teststats(data_merge$Score,data_merge$True.Score)
#df <- do.call(rbind, evaluation)
##print(df)
#df.eval <- data.frame("harrell_c"=evaluation$C,"hoslem_test"=evaluation$hoslem_p$pval, row.names = "value")
test_boot <- function(data, i){
 d2 <- data[i,]
 result <- teststats(d2$Score, d2$True.Score)
 return(result$C)
}

results <- boot(data=data_merge, statistic=test_boot,
   R=1000)
print("harrell_c")
...
test_boot_hoslem <- function(data, i){
 d2 <- data[i,]
 result <- teststats(d2$Score, d2$True.Score)
 return(result$hoslem_p$pval)
}

test_boot_hoslem_chi <- function(data, i){
 d2 <- data[i,]
 result <- teststats(d2$Score, d2$True.Score)
 return(result$hoslem_p$chi2)
}

hoslem <- boot(data=data_merge, statistic=test_boot_hoslem,
   R=1000)
print("hoslem")
#boot.ci(boot.out = hoslem,
#        type = c("norm", "basic",
#                 "perc", "bca"))

hoslem_chi <- boot(data=data_merge, statistic=test_boot_hoslem_chi,
   R=1000)
print("hoslem_c")
#boot.ci(boot.out = hoslem_chi,
#        type = c("norm", "basic",
#                 "perc", "bca"))

df.eval <- data.frame("harrell_c"=results$t,"hoslem_test"=hoslem$t, "hoslem_chi"=hoslem_chi$t)
