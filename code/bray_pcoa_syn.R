...
S.test <- read.csv(
  file = paste0(
    PARAM$folder.R,
    "input_challenge/test/pheno_test.csv"
  ),
  row.names = 1,
  check.names = FALSE
)
...
S.train <- read.csv(
  file = paste0(
    PARAM$folder.R,
    "input_challenge/train/pheno_training.csv"
  ),
  row.names = 1,
  check.names = FALSE
)
...
S.score <- read.csv(
  file = paste0(
    PARAM$folder.R,
    "input_challenge/scoring_nohide/pheno_scoring_nohide.csv"
  ),
  row.names = 1,
  check.names = FALSE
)
...
ggsave(
  filename = paste0(
    PARAM$folder.result,
    "pcoa_species_density_synthetic.pdf"
  ),
  plot = p.final,
  width = 7,
  height =6
