...
Due to participants’ limited access to the real dataset, we also worked with the top two performing teams to further improve the model performance and calibrations for the final submitted models after the challenge formally concluded. Top 2 performing models has submitted different version of model refinement and was evaluated in scoring dataset. All models were available in [TEAMS](https://github.com/FinriskHFDREAM/FinriskHF_post1/tree/main/TEAMS) directory. Among them we primarly reported are 2 modifications of SB2 final model and the ensemble model among all received model. We also checked if features selection (LASSO or EN) was applied in our Baseline All model which includes all the covariates information and the centered-log-ration microbiome composition. More details about the model variant during the post challenge periods was available in the Methods and Supplementary Information.

### 1. Baseline All model

We provided 3 examples of Baseline models based on Cox Proportional Hazard model to the participants together with their evaluation scores. Those three models were: a). **[Baseline Age-Sex](https://github.com/FinriskHFDREAM/FinriskHF_post1/blob/main/TEAMS/Baseline/BaseModel_AgeSex.R)**, b). **[Baseline Covariates](https://github.com/FinriskHFDREAM/FinriskHF_post1/blob/main/TEAMS/Baseline/BaseModel_AllCovar.R)**, and c). **[Baseline all](https://github.com/FinriskHFDREAM/FinriskHF_post1/blob/main/TEAMS/Baseline/BaseModel_MicrobesCovar.R)** which builds upon Baseline Covariates by further incorporating the centered log-ratio of relative species abundance information.
...
The ensembles model were generated by taking the mean of prediction scores obtained from the combined individual models as indicated in the figure.

The code to analyze available in [code/AveScore_allteam.R](https://github.com/FinriskHFDREAM/FinriskHF_post1/blob/main/code/AveScore_allteam.R) and the output is available in [results/pair_average_eval.csv](https://github.com/FinriskHFDREAM/FinriskHF_post1/blob/main/results/pair_average_eval.csv) as also provided in Supp. Table 8. 

### 4. Model evaluation using the dietary information and HF strict information

To assess the potential contribution of dietary information, we compared models with and without the Healthy Food Choices (HFC) score across three baseline model configurations. More information about the HFC score for FINRISK 2002 cohort was available in [Koponen et al., 2021](doi.org/10.1093/ajcn/nqab077). Additionally, we also performed sensitivity analysis using the strict definition of Heart Failure available on the FINRISK cohort (N cases = 288). The broad HF definition used in the challenge may have introduced some outcome misclassification because repeated furosemide purchases were included as an indicator of HF (Vuori et al. 2020). The broad definition was selected to prioritize sensitivity and maximize the number of HF events available for model development and evaluation. 
