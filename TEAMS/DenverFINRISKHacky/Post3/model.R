...
# Test only microbiome while omitting all clinical variables and their interactions
...
model <- function(
	...
	scores <- do.call("cbind", lapply(seeds, FUN=\(s){
		...
		catsystime("\nmodule_genus\n")
		module_genus <- module_glmnet(trainx = t(train_abugenus), trainy = train_y, testx = t(test_abugenus))
		...
		ensemble_temp[,"module_genus"] <- module_genus[[1]]
		...
		output_temp[,"module_genus"] <- module_genus[[2]]
		...
		coefs[[length(coefs)+1]] <<- list(
			...
			coefs_module_genus = module_genus[[3]],
			coefs_modules = tmp[[3]]
		)
