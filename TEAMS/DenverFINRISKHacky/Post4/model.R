...
# Testing a model without the modular structure but instead lumping all variables in a single optimization run
...
model <- function(
	...
	scores <- do.call("cbind", lapply(seeds, FUN=\(s){
		...
		catsystime("\nRunning all variables simultaneously without modular structure\n")
		wholemix <- module_glmnet(trainx = cbind(
				train_baseclin, train_clin, train_clin2, train_alpha, train_beta, train_relabus, train_curated1, train_curated2, train_tri, t(train_abuspecies), t(train_abugenus)
			), 
			trainy = train_y,
			testx = cbind(
				test_baseclin, test_clin, test_clin2, test_alpha, test_beta, test_relabus, test_curated1, test_curated2, test_tri, t(test_abuspecies), t(test_abugenus)
			)
		)
		...
		print("Aggregated regularized module prediction for test data prior to z-scaling:")
		print(wholemix[[2]])
