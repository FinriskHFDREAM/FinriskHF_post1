...
# Get the taxa names that passed filtering
taxa_keep <- taxa_names(train_relab)
# Subset the original count-based object (`train`) to match those taxa
train_filtered <- prune_taxa(taxa_keep, train)
test_filtered <- prune_taxa(taxa_keep, test)


# clr
print("Transforming into clr ...")
train_clr <-clr_transform(train_filtered)
test_clr <-clr_transform(test_filtered)


# Calculate co-abundances
print("Calculating co-abundances clusters ...")
coab_taxa <- co_abundances(train_filtered)
saveRDS(
  coab_taxa,
  file = file.path(
