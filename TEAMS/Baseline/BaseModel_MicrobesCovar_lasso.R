...
dir.create(file.path(PARAM$folder.R, "FINRISK_TEST_3_lasso"))
dir.create(file.path(PARAM$folder.R, "FINRISK_TEST_3_lasso","output"))
PARAM$folder.data <- paste0(PARAM$folder.R, "/")
PARAM$folder.result <- paste0(PARAM$folder.data, "FINRISK_TEST_3_lasso/output/")
...
# Extract selected coefficients (non-zero)
...
#nonzero.coefs <- lasso.coef[which(lasso.coef != 0)]
#selected.vars.df <- data.frame(
#    Feature = rownames(nonzero.coefs),
#    Coefficient = as.numeric(nonzero.coefs)
#)
# Save to CSV
#write.csv(selected.vars.df,
#          file = paste0(PARAM$folder.result, "lasso_selected_features_with_coefs.csv"),
#          row.names = FALSE)

survo <- 'Surv(Event_time,Event)~'
basemodel <- as.formula(paste0(survo, paste(selected.vars, collapse = '+')))
...
#grun.py -n "Base3_lasso" -c " ~/opt/miniconda3/bin/Rscript /homes/perawija/data_pande/DREAM/FINRISK2022/post_challenge/FinriskHF_post1/TEAMS/Baseline/BaseModel_MicrobesCovar_lasso.R /homes/perawija/data_pande/DREAM/FINRISK2022/post_challenge/FinriskHF_post1/input_real/"
