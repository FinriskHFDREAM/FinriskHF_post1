...
dir.create(file.path(PARAM$folder.R, "BaseAgeSexDiet"))
dir.create(file.path(PARAM$folder.R, "BaseAgeSexDi","output"))
...
diet_score <- c("FOOD_RECOMMENDED_CHOICHES")
basepredictors <- c("Age","Sex")

survo <- 'Surv(Event_time,Event)~'
basemodel <- as.formula(paste0(survo,paste(c(basepredictors,diet_score),collapse='+')))
...
write.csv(coef, file=paste0(PARAM$folder.result, 
                           "coef.csv"))
