...
dir.create(file.path(PARAM$folder.R, "FINRISK_TEST_3_RSF"))
dir.create(file.path(PARAM$folder.R, "FINRISK_TEST_3_RSF","output"))
PARAM$folder.data <- paste0(PARAM$folder.R, "/")
PARAM$folder.result <- paste0(PARAM$folder.data, "FINRISK_TEST_3_RSF/output/")
...
# === Define the features to use (without LASSO) ===
# Example: use all taxa from a processed CLR matrix
features_used <- colnames(df.train)[!(colnames(df.train) %in% c("Event_time", "Event"))]
# Alternatively: use a curated set of core features or taxa
# features_used <- core_taxa_names

# === Prepare data for RSF ===
train_data_rsf <- df.train[, c("Event_time", "Event", features_used)]
test_data_rsf  <- df.test[, features_used, drop = FALSE]
...
coef.df <- data.frame(Feature = features_used)
...
#grun.py -n "Base3_RSF" -c " ~/opt/miniconda3/bin/Rscript /homes/perawija/data_pande/DREAM/FINRISK2022/post_challenge/FinriskHF_post1/TEAMS/Baseline/BaseModel_MicrobesCovarRSF.R /homes/perawija/data_pande/DREAM/FINRISK2022/post_challenge/FinriskHF_post1/input_real/"
