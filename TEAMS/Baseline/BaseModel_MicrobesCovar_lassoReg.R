...
dir.create(file.path(PARAM$folder.R, "FINRISK_TEST_3_lassoReg"))
dir.create(file.path(PARAM$folder.R, "FINRISK_TEST_3_lassoReg","output"))
PARAM$folder.data <- paste0(PARAM$folder.R, "/")
PARAM$folder.result <- paste0(PARAM$folder.data, "FINRISK_TEST_3_lassoReg/output/")
...
saveRDS(cvfit, file = paste0(PARAM$folder.result, "glmnet_model.rds"))
# Prepare test data
x_test <- as.matrix(df.test[, colnames(x)])  # Ensure same columns as training

# Predict linear predictors (risk scores)
risk_scores <- predict(cvfit, newx = x_test, s = "lambda.min", type = "link")

# Save to CSV
res <- data.frame(SampleID = rownames(df.test), Score = as.numeric(risk_scores))
write.csv(res, file = paste0(PARAM$folder.result, "scores.csv"), row.names = FALSE)


#grun.py -n "Base3_lassoReg" -c " ~/opt/miniconda3/bin/Rscript /homes/perawija/data_pande/DREAM/FINRISK2022/post_challenge/FinriskHF_post1/TEAMS/Baseline/BaseModel_MicrobesCovar_lassoReg.R /homes/perawija/data_pande/DREAM/FINRISK2022/post_challenge/FinriskHF_post1/input_real/"
